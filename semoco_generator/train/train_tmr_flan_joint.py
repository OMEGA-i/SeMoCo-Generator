"""Train a Flan-T5 TMR with JOINT text+motion encoder training.

Unlike the frozen variant, both encoders are warm-started from tmr-soma-rp
and trained together.  Includes cross-modal KL matching original TMR.
"""
...
from .tmr_flan_joint_module import TMRFlanJointModule
...
def main() -> None:
    ...
    log(rank, f"[data] train={len(train_ds)} val={len(val_ds)} clip_dim={train_ds.clip_dim}")
    ...
    # ---- Build encoders (warm-start both from tmr-soma-rp) ------------------
    snap = _find_snapshot()
    ...
    # Text encoder (warm-start transformer from motion encoder, replace projection)
    ...
    motion_decoder = motion_decoder.to(device)
    # Freeze decoder — it acts as a fixed anchor preventing latent space drift.
    for p in motion_decoder.parameters():
        p.requires_grad = False

    module = TMRFlanJointModule(
    ...
    log(rank, f"[model] trainable params={module.num_parameters() / 1e6:.1f}M  "
              ...
              f"layers={mcfg.num_layers}  heads={mcfg.num_heads}  joint")
    ...
    def compute_val_metrics(core, loader, dev, amp_dtype):
        core.eval()
        total, total_contrastive, n = 0.0, 0.0, 0
        with torch.no_grad():
            for batch in loader:
                if n >= 50 * loader.batch_size:
                    break
                _, m = core(batch, dev, amp_dtype, step=0)
                bs = len(batch["motion_feat_valid"])
                total += float(m["loss"].item()) * bs
                total_contrastive += float(m["contrastive"].item()) * bs
                n += bs
        core.train()
        return {"val/ce_mean": total_contrastive / max(n, 1), "val/loss": total / max(n, 1)}
    ...
    def train_log_metrics(metrics, step, dt):
        return {
            ...
            "train/kl_prior": float(metrics["kl_prior"].item()),
            "train/kl_cross": float(metrics["kl_cross"].item()),
            "train/scale": float(metrics["scale"].item()),
        }

    # ---- Run ----------------------------------------------------------------
    out_dir = resolve_local_uri(args.out_dir or train_cfg.get("out_dir", "runs/tmr-soma-flan-joint-v3"))
    run_training(
    ...
    if rank == 0:
        ...
        log(rank, f"[export] text_encoder.pt + motion_encoder.pt saved")
